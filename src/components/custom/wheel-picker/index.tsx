/*************************************************************
 * wheel-picker 컴포넌트
 * - created by hwanee-k, 2025-06-10
 * - https://github.com/ncdai/react-wheel-picker
 * - 1.0.12 버전 랜더링 이슈로 소스받아서 커스텀함.
 *************************************************************/
'use client';
...
} from '@/components/custom/wheel-picker/types';
import { useControllableState } from '@/components/custom/wheel-picker/use-controllable-state';
...
const WheelPickerWrapper: React.FC<WheelPickerWrapperProps> = ({ className, children }) => {
  return (
...
const WheelPicker: React.FC<WheelPickerProps> = ({
  ...
  const renderWheelItems = useMemo(() => {
    const renderItem = (item: WheelPickerOption, index: number, angle: number) => (
      <li
        key={index}
  ...
  const [delayedRenderHighlightItems, setDelayedRenderHighlightItems] = React.useState<React.ReactNode[]>([]);

  useEffect(() => {
    const timer = setTimeout(() => {
      setDelayedRenderHighlightItems(renderHighlightItems);
    }, 100);

    return () => clearTimeout(timer);
  }, [renderHighlightItems]);

  const normalizeScroll = (scroll: number) => ((scroll % options.length) + options.length) % options.length;
  ...
  const initiateDragGesture = (event: MouseEvent | TouchEvent) => {
    try {
      ...
      containerRef.current?.addEventListener('touchmove', handleDragMoveEvent, passiveOpts);
  ...
  const handleDragStartEvent = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const isTargetValid = containerRef.current!.contains(e.target as Node) || e.target === containerRef.current;
  ...
  const handleDragEndEvent = useCallback(
    (event: MouseEvent | TouchEvent) => {
      ...
      const isDragging = dragingRef.current;
      const isTargetValid =
        containerRef.current!.contains(event.target as Node) || event.target === containerRef.current;
...
export { type WheelPickerClassNames } from '@/components/custom/wheel-picker/types';
